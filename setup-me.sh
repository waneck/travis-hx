...
case $SETUP in
	...
	cpp )
		...
		;;
	java )
		javac -version || install openjdk || install openjdk-7-jdk || exit 1
		retry haxelib git hxjava https://github.com/HaxeFoundation/hxjava
		javac -version || exit 1
		;;
	cs )
		mcs --version || install mono-mcs || exit 1
	...
	flash | as3 | swf | swf9 | swf8 )
		...
		if [ $SETUP = "as3" ]; then
			...
			java -version || install openjdk || install openjdk-7-jdk || exit 1
	...
	js )
		if [ $TOOLCHAIN = "default" ] || [ $TOOLCHAIN = "nodejs" ]; then
			node -v || install nodejs
		elif [ $TOOLCHAIN = "browser" ]; then
			phantomjs -v || install phantomjs
		fi
		;;
	* )
		;;
